...
def main():
    ...
    for k in k_range:
        ...
        # Evaluate the model
        corrects, total, accuracy = test_model(model, test_data, device, k)
        accuracy_history.append(accuracy)
        print('Testing accuracy: {} ({} out of {}) with k = {}'.format(accuracy, corrects, total, k))
        if accuracy > best_accuracy:
            best_k = k
            best_accuracy = accuracy

    print('The best k is {}, with accuracy = {}'.format(best_k, best_accuracy))
        
    # Draw the graph and save
    fig, ax = plt.subplots()
    ax.plot(k_range, accuracy_history)
    fig.savefig('./reports/figures/k_search.png')
    plt.close(fig)
